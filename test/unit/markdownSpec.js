// Extended from Brian Ford's spec
describe('btfMarkdown', function () {
  ...
  it('should produce unordered lists', function () {
    var elt = angular.element(['<btf-markdown>',
			      '* foo',
			      '* bar',
			      '</btf-markdown>'].join('\n'));
    $compile(elt)($rootScope);
    expect(elt.html()).toBe(['<ul>',
			     '<li>foo</li>',
			     '<li>bar</li>',
			     '</ul>'].join('\n'));
    expect(window.hacked).toBeUndefined();
  ...
  it('should produce headers', function () {
    var elt = angular.element(['<btf-markdown>',
			      '# foo #',
			      '## bar ##',
			      '</btf-markdown>'].join('\n'));
    $compile(elt)($rootScope);
    expect(elt.html()).toBe(['<h1>foo</h1>',
			     '',
			     '<h2>bar</h2>'].join('\n'));
    expect(window.hacked).toBeUndefined();
  });
